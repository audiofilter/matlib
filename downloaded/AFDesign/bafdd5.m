...
tstart = clock;
[numD,denD,numH,denH,nD,aD,eD,fpD] = afdd5(speck,ninc,filtype);
desnumb=1;
destype='D5';
eseconds=etime(clock,tstart);
afddinfo;
clf;
moreaxis=afdplot(numD,denD,speca,filnumb,moreaxis,'D5');
sizespeca = length(speca);
if sizespeca==4
  title([sprintf('D5: Sa= %11.1d Hz %11.1d Hz %9.1d dB %9.1d dB',speca)],'FontSize',9);
elseif sizespeca==7
  title([sprintf('D5: Sa= %11.1d Hz %11.1d Hz %11.1d Hz %11.1d Hz %5.1d dB %9.1d dB %5.1d dB',speca)],'FontSize',7);
else
  title([sprintf('D5: %11.1d',speca)],'FontSize',10);
end
