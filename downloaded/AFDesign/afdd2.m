function [nD2,dD2,numH,denH,n,a,e,fp] = afdd2(speck,ninc,filtype)

% afdd2.m  AFD Design alternative D2
%   
...
%% calls:
%% afdnell.m, afdl.m, afdhp.m, afdsimsp.m
...
if filtype == 'l'
 ...
 nD2 = (ones(size(nn))/(2*pi*fp)).^nn .*numH;
 nd = length(denH)-1:-1:0;
 dD2 = (ones(size(nd))/(2*pi*fp)).^nd .*denH;
end
...
if filtype == 'h'
 ...
 nD2 = (ones(size(nn))/(2*pi*fp)).^nn .*nD1h;
 nd = length(dD1h)-1:-1:0;
 dD2 = (ones(size(nd))/(2*pi*fp)).^nd .*dD1h;
end
