function [nD4a,dD4a,numH,denH,n,a,e,fp] = afdd4a(speck,ninc,filtype)

% afdd4a.m  Design alternative D4a
%   
...
%% calls:
%% afdnell.m, afdxmax.m, afdhp.m, afdsimsp.m
...
if filtype == 'l'
 ...
 nD4a = (ones(size(nn))/(2*pi*fp)).^nn .*numH;
 nd = length(denH)-1:-1:0;
 dD4a = (ones(size(nd))/(2*pi*fp)).^nd .*denH;
end
...
if filtype == 'h'
 ...
 nD4a = (ones(size(nn))/(2*pi*fp)).^nn .*nD1h;
 nd = length(dD1h)-1:-1:0;
 dD4a = (ones(size(nd))/(2*pi*fp)).^nd .*dD1h;
end
