function twoaxis=afdstop(num,den,speca,filnumb,moreaxis,Dtype)

% afdstop.m  AFD Plot attenuation in stopband
%   
...
A = -20*log10(abs(H)+A0);
plot(f, A,'b', [speca(2) 5*speca(2)], [speca(4) speca(4)], 'r:');
xlabel('frequency (Hz)');
...
A0 = 1/10^40*ones(size(f));
A = -20*log10(abs(H)+A0);
plot(f, A,'b', [0 speca(1)], [speca(4) speca(4)], 'r:');
xlabel('frequency (Hz)');
ylabel('attenuation (dB)');
a = axis;
a(1) = f1;
