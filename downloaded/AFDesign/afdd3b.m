function [nD3b,dD3b,numH,denH,n,a,e,fp] = afdd3b(speck,ninc,filtype)

% afdd3B.m  Design alternative D3b
%   
...
%% calls:
%% afdnell.m, afdxmin.m, afdhp.m, afdsimsp.m
...
if filtype == 'l'
 ...
 nD3b = (ones(size(nn))/(2*pi*fp)).^nn .*numH;
 nd = length(denH)-1:-1:0;
 dD3b = (ones(size(nd))/(2*pi*fp)).^nd .*denH;
end
...
if filtype == 'h'
 ...
 nD3b = (ones(size(nn))/(2*pi*fp)).^nn .*nD1h;
 nd = length(dD1h)-1:-1:0;
 dD3b = (ones(size(nd))/(2*pi*fp)).^nd .*dD1h;
end
