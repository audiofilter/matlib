function twoaxis=dfdpass(num,den,speca,filnumb,moreaxis,Dtype)

% dfdpass.m  AFD Plot attenuation in passband
%   
...
plot(f, A,'b', [0 speca(1)], [speca(3) speca(3)], 'r:', [0 speca(1)], [0 0], 'r:');
xlabel('frequency');
...
p = 2*pi*f;
A = -20*log10(abs(freqz(num,den,p)));
plot(f, A, 'b', [speca(2) 0.5], [speca(3) speca(3)], 'r:', [speca(2) 0.5], [0 0], 'r:');
xlabel('frequency');
ylabel('attenuation (dB)');
a = axis;
a(1) = f1;
...
p = 2*pi*f;
A = -20*log10(abs(freqz(num,den,p)));
plot(f,A,'b',[speca(2) speca(3)],[0 0],'r:', ...
