function [nD3a,dD3a,numH,denH,n,a,e,fp] = afdd3a(speck,ninc,filtype)

% afdd3a.m  Design alternative D3a
%   
...
if filtype == 'l'
 ...
 nD3a = (ones(size(nn))/(2*pi*fp)).^nn .*numH;
 nd = length(denH)-1:-1:0;
 dD3a = (ones(size(nd))/(2*pi*fp)).^nd .*denH;
end
...
if filtype == 'h'
 ...
 nD3a = (ones(size(nn))/(2*pi*fp)).^nn .*nD1h;
 nd = length(dD1h)-1:-1:0;
 dD3a = (ones(size(nd))/(2*pi*fp)).^nd .*dD1h;
end
