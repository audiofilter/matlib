...
%
%[Xn,Y,N,H,X,G,F] = SIGGEN_TFB(NN,A,C,P,Cg,M,Nn,B) returns the noisy signal 
...
function [recNoisySig,receivedSignal,noise,channel,inputSignal,gamma,fs,NEXTnoise, AWGN, trainSignalFX] = ...
   siggen_tfb(N,Nb,AWGNpower,channels,totalInputPower,codingGain,margin,bf,chbf)
   ...
   inputSignal=zeros(1,length(M*(N+Nb)));
   for i=1:M
       inputSignal(((i-1)*(N+Nb)+Nb+1):i*(N+Nb))=inputSignal_mid(((i-1)*N+1):i*N);
       inputSignal(((i-1)*(N+Nb)+1):((i-1)*(N+Nb)+Nb))=inputSignal_mid(((i-1)*N+N-Nb+1):i*N);
   end
