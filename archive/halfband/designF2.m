function [f2,q2] = designF2(fp,zetap,phi)
% [f2 q2] = designF2(fp,zetap,phi)		Design the F2 sub-filter
...
for product = (nsdmin+1)*n2min:(nsdmax+1)*n2min
	...
	[f2 q2] = bquantize( h2(n2+1:nsub+1), nsd );
	h2 = (1+delta2)*phi*[f2(n2:-1:1); f2];
	mag = abs( polyval(h2,passband) );
	if max(abs(mag-1)) < delta2;
	    success =1;
	    break;
