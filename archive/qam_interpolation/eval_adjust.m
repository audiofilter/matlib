...
RX_M = 8*over + 1
alpha = 0.18
freq_off = 0.001;  % 10% frequency error wrt IF
freq = pi/2*(1+freq_off);
gain = 1/2^8;
...
time_bits = 5;
timelev = 2^(time_bits-1);
t_inc = 2^(-time_bits);
%tx_time_off = -0.5-0.5*t_inc;
tx_time_off = -0.5-t_inc;
...
for l=1:np
  ...
  t_adj = tx_time_off;
  t_adj = floor(tx_time_off*timelev)/timelev
  t_err = tx_time_off - t_adj
