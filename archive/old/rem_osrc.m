function [h,ha] = rem_osrc(nfilt, wtx, alpha, os)
								% UNTESTED!!!!!!!
%REM_OSRC	
%	Parks-McClellan optimal equiripple FIR filter design modified to design
%	square root raised cosine filters with weighting function
% Added oversampling factor
%	B=rem_osrc(N,wtx,alpha,os) returns a length N+1 linear phase (real, symmetric
...
frac = 1.0/os;
delf = 0.0;		
ff = [0 0.5*frac*(1+alpha)-delf 0.5*frac*(1+alpha)+delf 1.0];  
aa = [1 1 0 0];
nomex=0;
...
% MODIFIED CODE
for t=1:ngrid
	if (grid(t)>(0.25*frac*(1-alpha)))
		if (grid(t)< (0.25*frac*(1+alpha)) )
			des(t) = des(t)*cos( (4*grid(t)*os - 1.0 + alpha)/(4*alpha)*pi );
			if (grid(t)<0.25*frac) 
				if (des(t)~=0)
%					wt(t) = wt(t)/cos( (4*grid(t) - 1.0 + alpha)/(4*alpha)*pi );
				end;
			end;
		end;
	end;
