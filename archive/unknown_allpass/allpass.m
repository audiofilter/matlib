...
%K=sqrt(0.3);
K=0.1;
hani=[1:pe se:P/2+1];hani=[hani P-fliplr(hani(2:length(hani)-1))+2];
Ws=[K*ones(1,pe) ones(1,P/2+2-se)];Ws=[Ws fliplr(conj(Ws(2:length(Ws)-1)))];
%[B,acoe]=butter(N,pee);
%[B,bcoe]=butter(M,pee);
...
D=[exp(-ii*tau*(0:se-2)*dw) zeros(1,P/2-se+2)];D=[D fliplr(conj(D(2:length(D)-1)))];
