...
pcon=[pcon E];
if t<0.0001 | abs(max(E)-E2)/E2<0.001
...
ED=abs(DD);
ed1=env(ED(1:pe),1,pe);
ed2=env(ED(pe+1:length(ED)),1,P/2+2-se);
ed1=ed1/sum(ed1)*pe;
ed2=ed2/sum(ed2)*(P/2+2-se);
Ws=[K*ed1 ed2];
end
%A=fft(acoe,P);
%A1=exp(-ii*N*dw*(0:P-1)).*conj(A)./A;
...
plot(w,unwrap(angle(hl)),[w(pe*2) w(pe*2)],[-4 4])
